...
animal_weights_absolute <- as_tibble(read_csv("C:/Users/ajb22/Documents/school/dacss_601/DACSS601Fall21/_data/animal_weight.csv"))
...
my_dataset <- read_csv("'fox', 3, 12.45, TRUE, 2010-01-01\n'hound', 5, 32.45, FALSE, 2010-01-01", 
...
gss_cat %>% mutate(rincome = fct_recode(rincome, "Less than $1000" = "Lt $1000")) %>%  mutate(rincome = fct_recode(rincome, "NA" = "Not applicable", "NA" = "Don't know", "NA" = "No answer", "NA" = "gRefused")) %>% ggplot(aes(x = rincome)) + geom_bar() + coord_flip() 
...
gss_cat %>% filter(!rincome %in% c("Not applicable", "Don't know", "No answer", "Refused")) %>% count(rincome)
 # lump groups outside the largest 8
gss_cat %>% filter(!rincome %in% c("Not applicable", "Don't know", "No answer", "Refused")) %>% mutate(rincome = fct_lump(rincome, n=8))  %>% count(rincome) 
